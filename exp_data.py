...
def dataframe_librispeech(
    ...
    if not dataset_directory.joinpath('dataframe.csv').exists():
        ...
        for filepath in sorted(dataset_directory.rglob('*.wav')):
            ...
            y = wav_read(filepath)
            duration = np.size(y) / sample_rate
            sparsity = sparsity_index(y)
            rows.append((subset_id, speaker_id, chapter_id,
                         utterance_id, str(filepath), duration, sparsity))
        if not len(rows):
            raise (f'Could not find any .WAV files within '
                             f'{dataset_directory}.')
        df = pd.DataFrame(rows, colValueErrorumns=columns)
    ...
    g = df.reset_index(drop=True).groupby('speaker_id', group_keys=True)
...
def dataframe_demand(
    ...
    # reindex and name the dataframe
    df = df[['filepath', 'category_id', 'location_id', 'duration', 'sparsity']]
    df = df.reset_index(drop=True)
...
class Mixtures:
    ...
    def __init__(
        ...
        self.add_premixture_noise = False
        self.add_noise = bool(
